...
alv = merge(alv, cep, by = "CEP", all.x = TRUE) %>%
    filter(DATA_EXPIRACAO == "***") %>%
    mutate(INICIO_ATIVIDADE = dmy(INICIO_ATIVIDADE))
...
server <- function(input, output) {
        ...
        output$plot4 <- renderPlot({
            ...
            cum_sum <- obj_list[[1]] %>% 
                arrange(INICIO_ATIVIDADE) %>% 
                mutate(order = 1, cumsum = cumsum(order))
            
            ggplot(cum_sum) + 
                geom_line(aes(x = INICIO_ATIVIDADE, 
                              y = cumsum), 
                          linetype = 4, 
                          size = 2) + 
                theme_minimal() +
                scale_x_date(date_breaks = "5 year", 
                             ...
                             limits = as.Date(c('1960-01-01','2021-08-11'))) +
                xlab("Company inauguration") +
                ylab("Cumulative sum")
        })
